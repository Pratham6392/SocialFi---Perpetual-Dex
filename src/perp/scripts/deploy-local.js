...
async function main() {
  ...
  // Create Decimal structs for virtual reserves
  const virtualBase = { value: ethers.utils.parseEther("1000") }; // 1000 ETH
  const virtualQuote = { value: ethers.utils.parseEther("2000000") }; // 2000000 USDC (price ~2000)
  ...
  const exchangeRouter = await ExchangeRouter.deploy(
    ...
    vaultAddress,
    accountBalance.address,
    vaultAddress // collateralToken (using vault as collateral for now)
  );
